...
class Pokemon
  def level=(value)
    # Ensure this logic only applies to player's Pokémon if PLAYER_ONLY is true
    return if LevelCapsEX::PLAYER_ONLY && $game_temp.in_battle && defined?($game_temp.battle_opponent) && $game_temp.battle_opponent
  ...
  def crosses_level_cap?
    return false if LevelCapsEX::PLAYER_ONLY && $game_temp.in_battle && defined?($game_temp.battle_opponent) && $game_temp.battle_opponent
...
class Battle
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    isOutsider = (pkmn.owner.id != pbPlayer.id || (pkmn.owner.language != 0 && pkmn.owner.language != pbPlayer.language))
    if isOutsider
    ...
    if (!trainerBattle? || !isBossTrainer?) && pkmn.crosses_level_cap? && LevelCapsEX.level_cap_mode == 2
      over_level_cap = true
      exp = (exp / 10).to_i
      exp = 1 if exp < 1
    end
    ...
    loop do   # For each level gained in turn...
      ...
      @scene.pbLevelUp(pkmn, battler, oldTotalHP, oldAttack, oldDefense, oldSpAtk, oldSpDef, oldSpeed)
  ...
  def isBossTrainer?
    return @opponent.is_a?(Array) && @opponent.any? { |trainer| ["LEADER", "BOSS"].include?(trainer.trainer_type.upcase) }
  end

  # Methode zum Auswählen des nächsten Pokémon
  def pbSwitchInBetween(idxBattler, checkLax = false)
    party = pbParty(1)  # Gegnerische Partei
    choices = []
    pbPartyOrder(1).each_with_index do |i, j|
      next if @battlers[idxBattler].pokemonIndex == j  # Überspringe das aktuelle Pokémon
      next if party[j].fainted?  # Überspringe besiegte Pokémon
      choices.push(j)
    end
    return nil if choices.empty?  # Wenn keine lebenden Pokémon übrig sind
    return choices.sample  # Zufällige Auswahl eines lebenden Pokémon
  end
end
