...
class Pokemon
  def has_field_skill?
    ...
    Settings::MISC_SKILLS.each do |skill|
      ...
      # return true if self.species_data.has_skill?(skill) || self.hasMove?(skill)
      return true if self.hasMove?(skill) ###
    end
...
class Trainer
  def get_pokemon_with_move(move)
    Settings::HM_SKILLS.each do |hm|
      if move == hm
        pokemon_party.each { |pkmn| return pkmn if pkmn.hasMove?(hm) ||  pkmn.species_data.tutor_moves.include?(hm) }
      end
    end
