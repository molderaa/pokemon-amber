    ...
    # if pbSaveScreen
      ...
      using(window = Window_TextEntry_Keyboard.new(starttext,0,0,240,64)){
        ...
        loop do
          ...
          pbUpdate
          if !@sprites["messagebox"].busy?
            if Input.triggerex?(:ESCAPE) || Input.trigger?(Input::BACK)
              ret=""  # Return an empty string if escape is pressed
              break
            elsif Input.triggerex?(:RETURN) || Input.trigger?(Input::USE)
              ret=window.text
              break
            end
          end
...
class CableClubScreen
  ...
  def pbConnectServer(partner_id)
    ...
    if pbConfirm("Would you like to attempt to connect to the server? This can take a while.")
    # puts "test" ###
  ...
  def do_battle(connection,team_order,partner_order)
    ...
    @battle_settings = nil
    pbSaveScreenCustom ### save directly after a battle
    if @client_id == 0
      choose_activity(connection)
    else
      await_choose_activity(connection)
  ...
  def do_trade(connection)
    ...
    pbSaveScreenCustom ### save directly after a trade
    resync=false
